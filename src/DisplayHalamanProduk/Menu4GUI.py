...
class Menu4GUI(tk.Tk):
    def __init__(roo):
        ...
        def displayMenu4():
            ...
            menu_name = Label(roo, text="Teh Panas\n", font=("Arial", 12, "bold"))
            menu_name.place(x=360, y=150)

            menu_details = Label(roo, text="Daun teh diseduh dengan air panas\n", font=("Arial", 10, "normal"))
            menu_details.place(x=360, y=180)

            menu_harga = Label(roo, text="Rp2.000\n", font=("Arial", 10, "bold"))
            ...
            buttonTambahPesanan = Button(roo, text="Tambahkan ke Pesanan",command=lambda: tambah.TambahProduk(4, spbox.get(), "Teh Panas", 2000.0), bg= '#FBB43C')
            ...
            def Hapus():
                conn = getMenu()
                cursor = conn.cursor()
                query = "DELETE FROM datapesanancustomer WHERE id_barang = '4' "
            ...
            def UbahKuantitas(jumlah_barang):
                conn = getMenu()
                cursor = conn.cursor()
                query = "UPDATE datapesanancustomer SET jumlah_barang=%s WHERE id_barang = '4' "
