...
class Menu6GUI(tk.Tk):
    def __init__(roo):
        ...
        def displayMenu6():
            ...
            paketAyam_name = Label(roo, text="Es Teh Manis/Teh manis\n", font=("Arial", 12, "bold"))
            paketAyam_name.place(x=600, y=470) # letaknya masih asal

            paketAyam_details = Label(roo, text="Teh panas/es teh ditambah gula\n", font=("Arial", 10, "normal"))
            paketAyam_details.place(x=600, y=495) # letaknya masih asal

            paketAyam_details2 = Label(roo, text="Rp4.000\n", font=("Arial", 10, "bold"))
            ...
            buttonTambahPesanan = Button(roo, text="Tambahkan ke Pesanan",command=lambda  : tambah.TambahProduk(6, spbox.get(), "Es Teh Manis/Teh manis", 4000.0), bg= '#FBB43C')
            ...
            def Hapus():
                conn = getMenu()
                cursor = conn.cursor()
                query = "DELETE FROM datapesanancustomer WHERE id_barang = '6' "
            ...
            def UbahKuantitas(jumlah_barang):
                conn = getMenu()
                cursor = conn.cursor()
                query = "UPDATE datapesanancustomer SET jumlah_barang=%s WHERE id_barang = '6' "
