...
class Menu1GUI(tk.Tk):
    def __init__(roo):
        ...
        def displayMenu1():
            ...
            buttonTambahPesanan = Button(roo, text="Tambahkan ke Pesanan",command=lambda: tambah.TambahProduk('1',spbox.get(),'Nasi Goreng Ayam',25000.0), bg= '#FBB43C')
            ...
            def Hapus():
                conn = getMenu()
                cursor = conn.cursor()
                query = "DELETE FROM datapesanancustomer WHERE id_barang = '1' "
            ...
            def UbahKuantitas(jumlah_barang):
                conn = getMenu()
                cursor = conn.cursor()
                query = "UPDATE datapesanancustomer SET jumlah_barang=%s WHERE id_barang = '1' "
